...
function AdminsTable(props) {
	...
	const admins = useSelector(({ accesscontrolApp }) => accesscontrolApp.admins.data);
	const searchText = useSelector(({ accesscontrolApp }) => accesscontrolApp.admins.searchText);
	...
	useEffect(() => {
		if (searchText.length !== 0) {
			setData(_.filter(admins, item => item.name.toLowerCase().includes(searchText.toLowerCase())));
			setPage(0);
		} else {
			setData(admins);
		}
	...
	function handleClick(item) {
		if(authUser.role == 'superadmin' ) {
			props.history.push(`/admins/${item.id}`);
		}
	}
